...
class ESMStructuralSplitDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        # if split and split_level and cv_partition:
        #
        #
        # else:
        #     with open(os.path.join(self.base_path, 'names.txt')) as f:
        #         self.names = f.read().splitlines()

        self.pkl_dir = os.path.join(self.base_path, 'pkl')
        self.msa_dir = os.path.join(self.base_path, 'msas')
        self.enc_dir = os.path.join(self.base_path, 'encodings')
        ...
        start, end = 0, 20
        self.number = 20
        values = torch.linspace(start, end, self.number + 2)
        self.step = values[1] - values[0]
        self.values = values[1:-1]
    ...
    def __getitem__(self, idx):
        ...
        distogram = torch.FloatTensor(obj['dist'])
        coords = torch.FloatTensor(obj['coords'])

        diff = (distogram.reshape(-1)[..., None] - self.values) / self.step
        labels = torch.argmin(torch.abs(diff), dim=-1)

        graph = Data(
            __num_nodes__=len(coords),
            pos=coords,
            edge_index=torch.nonzero((torch.ones_like(distogram)).type(torch.float)).T,
            edge_distance=distogram.reshape(-1),
            edge_labels=labels,
            name=name
        )

        if self.preembed:
            enc_finame = os.path.join(self.enc_dir, name[1:3], f'{name}.pkl')
            with open(enc_finame, 'rb') as f:
                (emb, att) = pickle.load(f)
            graph.enc = emb
            graph.edge_features = att

        if torch.any(~torch.isfinite(coords)):
            return self[random.randint(0, len(self)-1)]

        if self.get_msa:
            msa_fname = os.path.join(self.msa_dir, name[1:3], f'{name}.a3m')
            msa = self.read_msa(msa_fname, self.nseq)
        else:
            msa = None
